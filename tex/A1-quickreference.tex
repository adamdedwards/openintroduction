...
\iflabelexists{part:pred_logic}{  %Optional section if quantification logic is included
...
\begin{earg}
\item[] $(\script{A}\eand\script{B}) \iff (\script{B}\eand\script{A})$\\
\item[] $(\script{A}\eor\script{B}) \iff (\script{B}\eor\script{A})$\\
\item[] $(\script{A}\eiff\script{B}) \iff (\script{B}\eiff\script{A})$
\end{earg}


\noindent\textsc{DeMorgan} (DeM)
\begin{earg}
\item[] $\enot(\script{A}\eor\script{B}) \iff (\enot\script{A}\eand\enot\script{B})$\\
\item[] $\enot(\script{A}\eand\script{B}) \iff (\enot\script{A}\eor\enot\script{B})$
\end{earg}
